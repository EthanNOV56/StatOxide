...
impl Formula {
    ...
    pub fn parse(input: &str) -> Result<Self, String> {
        parse_formula(input).map_err(|e| e.to_string())
    }
    ...
    pub fn build_matrix(&self, df: &DataFrame) -> Result<Array2<f64>, String> {
        ...
        for var in &vars {
            if !df.column_names().contains(var) {
                return Err(format!("Variable '{}' not found in DataFrame", var));
            }
        }
    ...
    pub fn response_vector(&self, df: &DataFrame) -> Result<Option<Array1<f64>>, String> {
        if let Some(ref resp) = self.response {
            let resp_name = match resp {
                Term::Variable(name) => name,
                _ => return Err("Complex response terms not yet supported".to_string()),
            };
            
            let series = df.column(resp_name)
                .ok_or_else(|| format!("Response variable '{}' not found", resp_name))?;
...
fn build_term_matrix(term: &Term, df: &DataFrame) -> Result<Vec<Vec<f64>>, String> {
    match term {
        Term::Variable(name) => {
        ...
        Term::Function(func, arg) => {
            ...
            if base_cols.len() != 1 {
                return Err("Functions can only be applied to single variables".to_string());
            }
        ...
        Term::Polynomial(base, power) => {
            ...
            if base_cols.len() != 1 {
                return Err("Polynomial can only be applied to single variables".to_string());
            }
