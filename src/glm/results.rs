...
impl GLMResults {
    ...
    pub fn summary(&self, feature_names: &[String]) -> String {
        ...
        summary.push_str(&format!("Degrees of Freedom: {} total, {} residual\n",
        ...
        for i in 0..n_coef {
            ...
            summary.push_str(&format!("{:>20} {:>10.4} {:>10.4} {:>10.4} {:>10.4} {}\n",
    ...
    pub fn cooks_distance(&self) -> Array1<f64> {
        ...
        for i in 0..n {
            ...
            if hii < 1.0 && self.scale > 0.0 {
                cooks[i] = (pearson_resid.powi(2) / (p as f64 * self.scale)) * 
                          (hii / (1.0 - hii).powi(2));
            }
        }
    ...
    pub fn model_info(&self) -> String {
        format!(
            ...
            self.family.name(), self.link.name(), self.intercept, self.n_obs, self.n_params
        )
    }
}