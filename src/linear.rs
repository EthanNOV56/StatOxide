...
impl OLS {
    ...
    pub fn fit_formula(&self, formula: &Formula, df: &DataFrame) -> Result<LinearRegressionResults, String> {
        let X = formula.build_matrix(df)?;
        let y = formula.response_vector(df)?
            .ok_or_else(|| "Formula must include response variable".to_string())?;
        
        self.fit(&X, &y)
    }
    
    /// Fit model with design matrix X and response y
    pub fn fit(&self, X: &Array2<f64>, y: &Array1<f64>) -> Result<LinearRegressionResults, String> {
        let n = X.shape()[0];
        let p = X.shape()[1];
        
        if n <= p {
            return Err("Not enough observations for OLS estimation".to_string());
        }
        
        if y.len() != n {
            return Err(format!("X has {} rows, y has {} elements", n, y.len()));
        }
...
impl<'a> LinearModelBuilder<'a> {
    ...
    /// Fit OLS model
    pub fn ols(self) -> Result<LinearRegressionResults, String> {
    ...
    pub fn ridge(self, alpha: f64) -> Result<LinearRegressionResults, String> {
        let (X, y) = self.prepare_data()?;
    ...
    fn prepare_data(&self) -> Result<(Array2<f64>, Array1<f64>), String> {
        if let (Some(formula), Some(df)) = (self.formula, self.df) {
            ...
            Err("No data provided for model fitting".to_string())
        }
    }
}
