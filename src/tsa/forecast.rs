...
impl ForecastMetrics {
    ...
    pub fn new(actual: &Array1<f64>, predicted: &Array1<f64>) -> Result<Self> {
        ...
        if n != predicted.len() {
            return Err(StatError::DataError(
                format!("Actual and predicted lengths differ: {} vs {}", n, predicted.len())
            ));
        }
        
        if n == 0 {
            return Err(StatError::DataError("Empty data for forecast evaluation".to_string()));
        }
        ...
        for i in 0..n {
            ...
            if actual[i] != 0.0 {
                ...
                symmetric_errors[i] = (error.abs() / (actual[i].abs() + predicted[i].abs())) * 200.0;
            }
        }
        ...
        let mape = if abs_percentage_errors.iter().any(|&x| x.is_finite()) {
            abs_percentage_errors.iter()
                .filter(|&&x| x.is_finite())
                .sum::<f64>() / abs_percentage_errors.iter().filter(|&&x| x.is_finite()).count() as f64
        } else { 0.0 };
        
        let smape = if symmetric_errors.iter().any(|&x| x.is_finite()) {
            symmetric_errors.iter()
                .filter(|&&x| x.is_finite())
                .sum::<f64>() / symmetric_errors.iter().filter(|&&x| x.is_finite()).count() as f64
        } else { 0.0 };
    ...
    pub fn compare(&self, other: &Self, name_a: &str, name_b: &str) -> String {
        ...
        comparison.push_str(&format!("MAE:  {:.4} vs {:.4} ({:+.2}%)\n",
            self.mae, other.mae, (other.mae - self.mae) / self.mae.max(1e-10) * 100.0));
        comparison.push_str(&format!("RMSE: {:.4} vs {:.4} ({:+.2}%)\n",
            self.rmse, other.rmse, (other.rmse - self.rmse) / self.rmse.max(1e-10) * 100.0));
        comparison.push_str(&format!("MAPE: {:.2}% vs {:.2}% ({:+.2}pp)\n",
            self.mape, other.mape, other.mape - self.mape));
        comparison.push_str(&format!("MASE: {:.4} vs {:.4} ({:+.2}%)\n",
            self.mase, other.mase, (other.mase - self.mase) / self.mase.max(1e-10) * 100.0));
...
impl PredictionInterval {
    ...
    pub fn to_string(&self) -> String {
        format!("{:.4} [{:.4}, {:.4}] ({}%)", 
            self.point, self.lower, self.upper, (self.level * 100.0) as i32)
    }
}
...
impl PredictionIntervals {
    ...
    pub fn bootstrap(
        ...
        for i in 0..n {
            let mut values: Vec<f64> = bootstrap_forecasts.iter()
                .map(|arr| arr[i])
                .collect();
