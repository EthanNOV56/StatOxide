...
impl TSAResults {
    ...
    pub fn summary(&self) -> String {
        ...
        summary.push_str(&format!("  Jarque-Bera: {:.4} (p={:.4})\n", diag.jarque_bera.0, diag.jarque_bera.1));
        summary.push_str(&format!("  Ljung-Box(10): {:.4} (p={:.4})\n", diag.ljung_box.0, diag.ljung_box.1));
...
impl ResidualDiagnostics {
    ...
    pub fn summary(&self) -> String {
        ...
        summary.push_str(&format!("Normality (Jarque-Bera): {:.4} (p={:.4}) {}\n",
            self.jarque_bera.0, self.jarque_bera.1,
            if self.is_normal() { "✓" } else { "✗" }));
        summary.push_str(&format!("Autocorrelation (Ljung-Box): {:.4} (p={:.4}) {}\n",
            self.ljung_box.0, self.ljung_box.1,
            if !self.has_autocorrelation() { "✓" } else { "✗" }));
        summary.push_str(&format!("Durbin-Watson: {:.4} - {}\n",
            self.durbin_watson, self.durbin_watson_interpretation()));
        
        summary.push_str(&format!("\nDistribution:\n"));
        summary.push_str(&format!("  Mean: {:.6}\n", self.mean));
        summary.push_str(&format!("  Std Dev: {:.6}\n", self.std_dev));
        summary.push_str(&format!("  Skewness: {:.4} ({})\n",
            self.skewness, if self.skewness.abs() > 0.5 { "skewed" } else { "symmetric" }));
        summary.push_str(&format!("  Kurtosis: {:.4} ({})\n",
            self.kurtosis, if self.kurtosis > 3.5 { "leptokurtic" } else if self.kurtosis < 2.5 { "platykurtic" } else { "mesokurtic" }));
...
impl ModelComparison {
    ...
    pub fn summary(&self) -> String {
        let mut summary = String::new();
        summary.push_str(&format!("Model Comparison: {} vs {}\n", self.model_a, self.model_b));
        summary.push_str(&format!("{}\n", "=".repeat(self.model_a.len() + self.model_b.len() + 17)));
        
        summary.push_str(&format!("AIC difference: {:.4} ({} preferred)\n",
            self.aic_diff, self.aic_preferred()));
        summary.push_str(&format!("  Evidence: {}\n", self.aic_evidence()));
        
        summary.push_str(&format!("BIC difference: {:.4} ({} preferred)\n",
            self.bic_diff, self.bic_preferred()));
