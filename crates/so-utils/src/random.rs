...
use rand::distributions::{Distribution, Uniform, Normal as RandNormal};
...
pub fn random_uniform_array(n: usize, low: f64, high: f64) -> Array1<f64> {
    ...
    Array1::from_iter((0..n).map(|_| dist.sample(&mut rng)))
}
...
impl SeededRng {
    ...
    pub fn uniform_array(&mut self, n: usize, low: f64, high: f64) -> Array1<f64> {
        let dist = Uniform::new(low, high);
        Array1::from_iter((0..n).map(|_| dist.sample(&mut self.rng)))
    }
