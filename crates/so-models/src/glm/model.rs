...
impl GLMModelBuilder {
    ...
    pub fn build(self) -> GLM {
        ...
        if !is_valid_link(self.family, link) {
            panic!(
                "Invalid link-function combination: {} with {}",
                self.family.name(),
                link.name()
            );
        }
...
impl GLM {
    ...
    pub fn fit(&self, formula: &str, data: &DataFrame) -> Result<GLMResults> {
        ...
        let response_var = formula
            ...
            .ok_or_else(|| Error::DataError("Response must be a simple variable".to_string()))?;

        let y_series = data.column(&response_var).ok_or_else(|| {
    ...
    fn fit_irls(&self, X: &Array2<f64>, y: &Array1<f64>) -> Result<GLMResults> {
        ...
        if n <= p {
            return Err(Error::DataError(format!(
                "Not enough observations (n={}) for p={} parameters",
                n, p
            )));
        }
        ...
        if !converged {
            return Err(Error::Message(format!(
                "IRLS did not converge after {} iterations",
                self.max_iter
            )));
        }
        ...
        let pearson_residuals: Array1<f64> = y
            .iter()
            .zip(fitted.iter())
            .map(|(&y_val, &mu_val)| {
