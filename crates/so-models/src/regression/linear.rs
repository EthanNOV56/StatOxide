...
impl OLS {
    ...
    pub fn fit_formula(
        ...
        formula: &Formula,
        df: &DataFrame,
    ) -> Result<LinearRegressionResults> {
    ...
    pub fn fit(&self, X: &Array2<f64>, y: &Array1<f64>) -> Result<LinearRegressionResults> {
        ...
        if n <= p {
            return Err(Error::Message(
                "Not enough observations for OLS estimation".to_string(),
            ));
        }

        if y.len() != n {
            return Err(Error::DimensionMismatch(format!(
                "X has {} rows, y has {} elements",
                n,
                y.len()
            )));
        }
        ...
        let (f_statistic, f_p_value) = if self.intercept && df_model > 0 && df_residual > 0 {
            ...
            if (1.0 - r_squared).abs() < f64::EPSILON {
                ...
                let f_stat =
                    (r_squared / df_model as f64) / ((1.0 - r_squared) / df_residual as f64);
                ...
                if f_stat.is_nan() || f_stat.is_infinite() {
                    ...
                    let x_safe = if x_clamped <= 0.0 {
                        f64::MIN_POSITIVE
                    } else if x_clamped >= 1.0 {
                        1.0 - f64::EPSILON
                    } else {
                        x_clamped
                    };
                    ...
                    let beta_val = statrs::function::beta::beta_reg(
                        df_model as f64 / 2.0,
    ...
    fn solve_normal_equations(&self, X: &Array2<f64>, y: &Array1<f64>) -> Result<Array1<f64>> {
        ...
        so_linalg::solve(&xtx, &xty).map_err(|e| {
            Error::LinearAlgebraError(format!("Solving normal equations failed: {}", e))
        })
    }
    ...
    fn compute_inference(
        ...
        let std_errors: Array1<f64> = (0..p).map(|i| cov_matrix[(i, i)].sqrt()).collect();
...
impl Ridge {
    ...
    pub fn fit(&self, X: &Array2<f64>, y: &Array1<f64>) -> Result<LinearRegressionResults> {
        ...
        let coefficients_std = match so_linalg::solve(&xtx_regularized, &xty) {
            Ok(coef) => coef,
            Err(e) => {
                return Err(Error::LinearAlgebraError(format!(
                    "Ridge regression solve failed: {}",
                    e
                )));
            }
        };
        ...
        let coefficients =
            self.unstandardize_coefficients(&coefficients_std, &x_mean, &x_std, y_mean);
...
impl<'a> LinearModelBuilder<'a> {
    ...
    fn prepare_data(&self) -> Result<(Array2<f64>, Array1<f64>)> {
        if let (Some(formula), Some(df)) = (self.formula, self.df) {
            let X = formula.build_matrix(df)?;
            let y = formula.response_vector(df)?.ok_or_else(|| {
                Error::Message("Formula must include response variable".to_string())
            })?;
            Ok((X, y))
            ...
            Err(Error::Message(
                "No data provided for model fitting".to_string(),
            ))
        }
    }
}
...
mod tests {
    ...
    fn test_ols_basic() {
        ...
        let X = arr2(&[[1.0, 1.0], [1.0, 2.0], [1.0, 3.0], [1.0, 4.0], [1.0, 5.0]]);
    ...
    fn test_ridge_basic() {
        ...
        println!(
            "Differences: intercept={:.4}, slope={:.4}",
            intercept_diff, slope_diff
        );

        assert!(
            intercept_diff < 0.25,
            "Intercept coefficient {} not close to 2.0 (diff={})",
            results.coefficients[0],
            intercept_diff
        );
        assert!(
            slope_diff < 0.25,
            "Slope coefficient {} not close to 3.0 (diff={})",
    ...
    fn test_model_builder() {
        ...
        let results = LinearModelBuilder::formula(&formula, &df).ols().unwrap();
