...
impl LinearMixedModelBuilder {
    ...
    pub fn fit(self) -> Result<LMMResults> {
        ...
        let y = self.data.column(&self.response).ok_or_else(|| {
            Error::DataError(format!("Response column '{}' not found", self.response))
        })?;
    ...
    fn build_random_design_matrices(&self) -> Result<(Vec<Array2<f64>>, Vec<usize>)> {
        ...
        for random_effect in &self.random_effects {
            ...
            let group_col = self.data.column(&random_effect.group_var).ok_or_else(|| {
                Error::DataError(format!(
                    "Group column '{}' not found",
                    random_effect.group_var
                ))
            })?;
    ...
    fn fit_em(
        ...
        while !converged && iter < self.max_iter {
            ...
            beta = solve(&XtVX, &XtVy).map_err(|e| {
                Error::LinearAlgebraError(format!("Failed to solve for beta: {}", e))
            })?;
        ...
        let V_inv = self.compute_V_inv(&Z, sigma2_e, &sigma2_u, group_sizes)?;
        let XtVX = X.t().dot(&V_inv.dot(X));
        let cov_beta = inv(&XtVX).map_err(|e| {
            Error::LinearAlgebraError(format!("Failed to invert X'V^{{-1}}X: {}", e))
        })?;
...
impl GLMMBuilder {
    ...
    fn fit_pql(&self, y: &Array1<f64>, link: Link) -> Result<GLMMResults> {
        ...
        while !converged && iter < self.max_iter {
            ...
            for i in 0..n {
                let d_eta_d_mu = if mu_eta[i].abs() > 1e-10 {
                    1.0 / mu_eta[i]
                } else {
                    1.0
                };
            ...
            let solution = solve(&M, &rhs).map_err(|e| {
                Error::LinearAlgebraError(format!("Failed to solve mixed model equations: {}", e))
            })?;
            ...
            let pearson_residuals =
                residuals.mapv(|r| r * r / self.family.variance(new_mu[0]).max(1e-10));
        ...
        let Minv = inv(&M).map_err(|e| {
            Error::LinearAlgebraError(format!("Failed to invert mixed model matrix: {}", e))
        })?;
