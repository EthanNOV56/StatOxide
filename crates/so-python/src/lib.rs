...
// Import models
use so_models::glm::{GLM as RustGLM, GLMModelBuilder, GLMResults, Family, Link};
...
impl PySeries {
    ...
    fn __repr__(&self) -> String {
        format!("Series(name='{}', len={})", self.inner.name(), self.inner.len())
    }
}
...
impl PyDataFrame {
    ...
    fn new(data: HashMap<String, Vec<f64>>) -> PyResult<Self> {
        ...
        match DataFrame::from_series(columns) {
            ...
            Err(e) => Err(PyValueError::new_err(format!("Error creating DataFrame: {:?}", e))),
        }
    }
    ...
    fn get_column(&self, name: &str) -> PyResult<PySeries> {
        match self.inner.column(name) {
            Some(series) => Ok(PySeries { inner: series.clone() }),
    ...
    fn with_column(&mut self, series: &PySeries) -> PyResult<()> {
        let df = self.inner.clone().with_column(series.inner.clone())
    ...
    /// String representation
    fn __repr__(&self) -> String {
        format!("DataFrame(rows={}, cols={})", self.inner.n_rows(), self.inner.n_cols())
    }
}
...
impl PyFormula {
    ...
    fn new(formula: String) -> PyResult<Self> {
        match Formula::parse(&formula) {
            ...
            Err(e) => Err(PyValueError::new_err(format!("Error parsing formula: {:?}", e))),
        }
    }
...
impl PyFamily {
    ...
    #[staticmethod]
    fn gaussian() -> Self {
        PyFamily { inner: Family::Gaussian }
    }
    
    /// Create Binomial family
    #[staticmethod]
    fn binomial() -> Self {
        PyFamily { inner: Family::Binomial }
    }
    
    /// Create Poisson family
    #[staticmethod]
    fn poisson() -> Self {
        PyFamily { inner: Family::Poisson }
    }
    
    /// Create Gamma family
    #[staticmethod]
    fn gamma() -> Self {
        PyFamily { inner: Family::Gamma }
    }
    
    /// Create Inverse Gaussian family
    #[staticmethod]
    fn inverse_gaussian() -> Self {
        PyFamily { inner: Family::InverseGaussian }
    }
...
impl PyGLM {
    ...
    fn fit(&self, formula: &str, data: &PyDataFrame) -> PyResult<PyGLMResults> {
        match self.inner.fit(formula, &data.inner) {
            ...
            Err(e) => Err(PyRuntimeError::new_err(format!("GLM fitting failed: {:?}", e))),
        }
    }
    ...
    fn fit_matrix(&self, x: Vec<Vec<f64>>, y: Vec<f64>) -> PyResult<PyGLMResults> {
        ...
        if n_rows != y.len() {
            return Err(PyValueError::new_err("X and y must have same number of rows"));
        }
...
impl PyGLMResults {
    ...
    fn predict(&self, x: Vec<Vec<f64>>) -> PyResult<Vec<f64>> {
        ...
        if n_cols != self.inner.coefficients.len() {
            return Err(PyValueError::new_err(format!(
                ...
                n_cols, self.inner.coefficients.len()
            )));
        }
        ...
        for row in x {
            if row.len() != n_cols {
                return Err(PyValueError::new_err("All rows must have same number of columns"));
            }
            let mut pred = 0.0;
    ...
    fn summary(&self) -> String {
        ...
        format!(
            "GLM Results:\n  Coefficients: {:?}\n  AIC: {:.2}\n  BIC: {:.2}\n  Deviance: {:.2}\n  Scale: {:.2}",
            self.coefficients(), self.aic(), self.bic(), self.deviance(), self.scale()
        )
    }
}
...
impl PyTimeSeries {
    ...
    fn from_dataframe(df: &PyDataFrame, value_col: &str, date_col: &str) -> PyResult<Self> {
        match TimeSeries::from_dataframe(&df.inner, value_col, date_col) {
            ...
            Err(e) => Err(PyRuntimeError::new_err(format!("Failed to create TimeSeries: {:?}", e))),
        }
    }
...
impl PyARIMAResults {
    ...
    fn summary(&self) -> String {
        format!(
            "ARIMA Results:\n  AIC: {:.2}\n  BIC: {:.2}\n  Log-Likelihood: {:.2}\n  Sigma2: {:.4}",
            self.aic(), self.bic(), self.log_likelihood(), self.sigma2()
        )
    }
}
...
fn descriptive_summary(py: Python, data: Vec<f64>) -> PyResult<Py<PyDict>> {
    ...
    if !data.is_empty() {
        ...
        if data.len() >= 2 {
            ...
            // Min and max
            if let (Some(min), Some(max)) = (data.iter().min_by(|a, b| a.partial_cmp(b).unwrap()), 
...
fn linear_regression(py: Python, x: Vec<Vec<f64>>, y: Vec<f64>) -> PyResult<Py<PyDict>> {
    ...
    let n_cols = x[0].len();
    let x_array = ndarray::Array2::from_shape_vec((n_rows, n_cols), x.into_iter().flatten().collect())
    ...
    match model.fit(&x_array, &y_array) {
        ...
        Err(e) => Err(PyRuntimeError::new_err(format!("Linear regression failed: {:?}", e))),
    }
}
