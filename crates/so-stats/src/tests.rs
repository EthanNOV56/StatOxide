...
pub fn t_test_one_sample(data: &Array1<f64>, mu: f64, alternative: Alternative) -> Result<TestResult> {
    let n = data.len();
    if n < 2 {
        return Err(Error::DataError("Need at least 2 observations for t-test".to_string()));
    }
...
pub fn t_test_two_sample(
    ...
    if n1 < 2 || n2 < 2 {
        return Err(Error::DataError("Need at least 2 observations in each group".to_string()));
    }
    ...
    let pooled_var = ((n1 as f64 - 1.0) * var1 + (n2 as f64 - 1.0) * var2) / (n1 as f64 + n2 as f64 - 2.0);
...
pub fn t_test_paired(
    ...
    if x.len() != y.len() {
        return Err(Error::DataError("Paired samples must have same length".to_string()));
    }
...
pub fn chi_square_goodness_of_fit(
    ...
    if observed.len() != expected.len() {
        return Err(Error::DataError("Observed and expected arrays must have same length".to_string()));
    }
...
pub fn chi_square_test_independence(
    ...
    if n_rows < 2 || n_cols < 2 {
        return Err(Error::DataError("Contingency table must be at least 2x2".to_string()));
    }
...
pub fn shapiro_wilk_test(data: &Array1<f64>) -> Result<TestResult> {
    ...
    let n = data.len();
    if n < 3 {
        return Err(Error::DataError("Need at least 3 observations for Shapiro-Wilk test".to_string()));
    }
...
pub fn anova_one_way(groups: &[Array1<f64>]) -> Result<TestResult> {
    if groups.len() < 2 {
        return Err(Error::DataError("Need at least 2 groups for ANOVA".to_string()));
    }
    ...
    for group in groups {
        let n = group.len();
        if n < 2 {
            return Err(Error::DataError("Each group must have at least 2 observations".to_string()));
        }
    ...
    let ss_between: f64 = group_sizes.iter().zip(&group_means)
