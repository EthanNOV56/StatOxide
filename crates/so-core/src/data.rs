...
impl DataFrame {
    ...
    pub fn from_series(columns: HashMap<String, Series>) -> Result<Self, super::error::Error> {
        let mut n_rows = 0;
        for (name, series) in &columns {
            if n_rows == 0 {
                n_rows = series.len();
            } else if series.len() != n_rows {
                return Err(super::error::Error::DimensionMismatch(format!(
    ...
    pub fn select(&self, col_names: &[&str]) -> Result<Self, super::error::Error> {
    ...
    pub fn filter(&self, mask: &[bool]) -> Result<Self, super::error::Error> {
    ...
    pub fn design_matrix(&self, formula: &Formula) -> Result<Array2<f64>, super::error::Error> {
...
pub trait FromData<T> {
    fn from_data(data: T) -> Result<Self, super::error::Error>
    where
        Self: Sized;
}
...
pub trait ToData<T> {
    fn to_data(&self) -> Result<T, super::error::Error>;
}

impl FromData<Vec<Vec<f64>>> for DataFrame {
    fn from_data(data: Vec<Vec<f64>>) -> Result<Self, super::error::Error> {
        if data.is_empty() {
            return Ok(Self::new());
        }
